...
function initializeOLGA(board_container_id, board_id, notes_id, score_box_id, variation_container_id, variation_box_id, status_id, status_t_id, auto_button_id, alpha_id, numeric_id) {
  ...
  var onDragMove = function(newLocation, oldLocation, source,
    piece, position, orientation, target) {
    console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
  ...
  function onMoveEnd(oldPos, newPos) {
    ...
    // console.log(OLGA_MAIN.board.fen());
    // console.log(OLGA_MAIN.board.position());
    // console.log(OLGA_MAIN.variationList);
    // console.log('Move animation complete:')
    // console.log('Old position: ' + OLGA_MAIN.board.position(newPos))
    // console.log('New position: ' + OLGA_MAIN.board.fen(newPos))
    // console.log('New position: ' + OLGA_MAIN.board.position(oldPos))
    // console.log('Old position: ' + ChessBoard.position)
    // console.log('New position: ' + ChessBoard.fen(oldPos))
    // console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
  }
  ...
  var onDrop = function(source, target) {
    ...
    var isPawnMove = OLGA_MAIN.engine.get(source).type === "p";
    var isPawnPromotion = isPawnMove && target.match(/[18]$/);
    // console.log(!!onDragMove(source));
    if (isPawnPromotion) {
      var pieceColor = OLGA_MAIN.engine.get(source).color;
      // console.log(pieceColor);
      $("#pawn-promotion-selection")
        .attr("data-olga-source", source)
        .attr("data-olga-target", target)
        .show();
      $(".piece-promotion img").each(function() {
        var newSrc = $(this).prop("src").replace(/\/[wb]/, "/" + pieceColor);
        $(this).prop("src", newSrc)
      })
    } else {
      formerOnDrop(source, target)
    }
  };

  var formerOnDrop = function(source, target) {
    var move = OLGA_MAIN.engine.move({
      from: source,
      to: target,
      promotion: 'q' // NOTE: always promote to a queen for simplicity
    });
    // console.log(OLGA_MAIN.engine.moves()[49] === OLGA_MAIN.engine.move());
    // illegal move
  ...
  var formerOnDropPromotion = function(source, target, promotionValue) {
    var move = OLGA_MAIN.engine.move({
      from: source,
      to: target,
      promotion: promotionValue // NOTE: always promote to a queen for simplicity
    });
    // illegal move
  ...
  var cfg = {
    ...
    onMoveEnd: onMoveEnd,
    formerOnDropPromotion: formerOnDropPromotion,
    onSnapEnd: onSnapEnd,
...
function OLGA(board_container_id, board_id, notes_id, score_box_id, variation_container_id, variation_box_id, status_id, status_t_id, auto_button_id, alpha_id, numeric_id, cfg) {
  ...
  if (cfg === undefined) {
    ...
    this.formerOnDropPromotion = function() {};
    ...
    ocfg = { // DEFAULT OLGA config
      ...
      formerOnDropPromotion: this.formerOnDropPromotion,
    ...
    this.onDragMove = cfg.onDragMove;
    this.onMoveEnd = cfg.onMoveEnd;
    this.formerOnDropPromotion = cfg.formerOnDropPromotion;
  ...
  if (pgn_str.length !== 0) { //if a pgn was available
    var result = this.engine.load_pgn(pgn_str, {
      sloppy: true
    });
...
function setNewPGN(pgn) {
  ...
  OLGA_MAIN.history = OLGA_MAIN.engine.history({
    verbose: true
  });
...
function synchronizeOLGA() {
  ...
  if (OLGA_MAIN.sessionID !== "") {
    ...
    $.get("http://www.chessgames.com/perl/user_api", data_str, function(reply) {
      console.log(reply);
    }, "json");
  }
}
...
function toggleNotation() {
  ...
  var cfg = {
    ...
    onDragMove: OLGA_MAIN.onDragMove,
    onMoveEnd: OLGA_MAIN.onMoveEnd,
    formerOnDropPromotion: OLGA_MAIN.formerOnDropPromotion,
...
function copyFEN() {
  ...
  try {
    ...
    if (successful) {
      ...
      sbar.className = "show";
      setTimeout(function() {
        sbar.className = sbar.className.replace("show", "");
      }, 2000);
    }
  } catch (err) {}
}
